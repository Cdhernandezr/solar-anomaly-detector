...
class ModelTrainer:
    ...
    def __init__(self, model, train_loader, val_loader, test_loader):
        ...
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")#En este caso se usa siempre CPU por la configuracion del entorno
    ...
    def train_model(self, epochs=Config.MAX_EPOCHS, mlflow_logging=False):
        ...
        for epoch in range(epochs):
            ...
            if mlflow_logging:
                mlflow.log_metrics({
                    "train_loss_epoch": train_loss, "train_acc_epoch": train_acc, 
                    "val_loss_epoch": val_loss, "val_acc_epoch": val_acc
                }, step=epoch)
    ...
    def evaluate_on_test(self):
        ...
        report_dict = classification_report(all_labels, all_preds,
                                             target_names=['Normal', 'Defective'], output_dict=True)
...
def run_training():
    ...
    con seguimiento de experimentos de MLflow.
    """
    Logger.section("INICIANDO PROCESO DE ENTRENAMIENTO CON MLFLOW")
    ...
    # INICIO DEL EXPERIMENTO CON MLFLOW
    # Establece un nombre para tu grupo de experimentos. MLflow lo creará si no existe.
    mlflow.set_experiment("Deteccion de Anomalias Solares")

    with mlflow.start_run() as run:
        Logger.log(f"MLflow Run ID: {run.info.run_id}", 'info')
        
        # 2.1. Registrar Parámetros
        # Guardamos la configuración clave de este experimento.
        mlflow.log_params({
            "model_architecture": "ThermalNet",
        ...
        # Llamamos a la función de entrenamiento, activando el logging por época.
        trainer.train_model(mlflow_logging=True)
        
        # EVALUACIÓN Y REGISTRO DE RESULTADOS
        Logger.section("EVALUANDO MODELO FINAL")
        # Capturamos las métricas y la figura que ahora devuelve la función.
        final_metrics, confusion_matrix_fig = trainer.evaluate_on_test()
        
        # 2.2. Registrar Métricas Finales
        # Guardamos los resultados clave del rendimiento del modelo.
        mlflow.log_metrics({
        ...
        # 2.3. Registrar Artefactos
        # Guardamos la gráfica de la matriz de confusión como una imagen.
        mlflow.log_figure(confusion_matrix_fig, "confusion_matrix.png")
        
        # Guardamos el modelo entrenado.
        mlflow.pytorch.log_model(trainer.model, "model")

        Logger.log("✅ Modelo, métricas y artefactos registrados en MLflow.", 'success')
